...
def frequent_calls(number: str, db: Session = Depends(get_db)):
    try:
        ...
        payload = {"number": number, "results": data}
        payload.update(AnalysisService.detect_risk(db, payload))
        return payload
...
def common_contacts(
    ...
    try:
        ...
        payload = {"num1": num1, "num2": num2, "common_contacts": data}
        payload.update(AnalysisService.detect_risk(db, payload))
        return payload
    except Exception as exc:
...
def movement_tracking(number: str, db: Session = Depends(get_db)):
    try:
        ...
        payload = {"number": number, "movement": data}
        payload.update(AnalysisService.detect_risk(db, payload))
        return payload
    except Exception as exc:
...
def crime_tower_match(crime_id: int, db: Session = Depends(get_db)):
    try:
        ...
        payload = {"crime": crime, "nearby_numbers": numbers}
        payload.update(AnalysisService.detect_risk(db, payload))
        return payload
